...
namespace DirectXGame
{
	...
	const DirectX::XMFLOAT2 SpawnManager::SpriteScale = XMFLOAT2(3.0f, 3.0f);
	const uint32_t SpawnManager::SpriteColumns = 3;
	const uint32_t SpawnManager::SpriteRows = 1;
	const XMFLOAT2 SpawnManager::UVScalingFactor = XMFLOAT2(1.0f / SpriteColumns, 1.0f / SpriteRows);
	const std::uint32_t SpawnManager::SpawnLifeTime = 2500;

	SpawnManager::SpawnManager(const shared_ptr<DX::DeviceResources>& deviceResources, const shared_ptr<Camera>& camera, std::uint32_t maxSpawns) :
		...
		mRandomGenerator(static_cast<uint32_t>(chrono::system_clock::now().time_since_epoch().count())), mMaxSpawns(maxSpawns)
	{
	}
	...
	void SpawnManager::CreateDeviceDependentResources()
	{
		...
		auto loadSpriteSheetAndCreateSpritesTask = (createPSTask && createVSTask).then([this]() {
			ThrowIfFailed(CreateWICTextureFromFile(mDeviceResources->GetD3DDevice(), L"Content\\Textures\\Shapes.png", nullptr, mSpriteSheet.ReleaseAndGetAddressOf()));
			InitializeVertices();
			InitializeSprites();
		});
		...
		loadSpriteSheetAndCreateSpritesTask.then([this]() {
			mLastSpawnTime = chrono::system_clock::now();
	...
	void SpawnManager::Update(const StepTimer&)
	{
		if (!mLoadingComplete)
		{
			return;
		...
		bool updatePosition = (chrono::duration_cast<chrono::milliseconds>(chrono::system_clock::now() - mLastSpawnTime).count() > SpawnLifeTime);
	...
	void SpawnManager::Render(const StepTimer&)
	{
	...
	void SpawnManager::DrawSprite(Sprite& sprite)
	{
		ID3D11DeviceContext* direct3DDeviceContext = mDeviceResources->GetD3DDeviceContext();
	...
	void SpawnManager::InitializeVertices()
	{
	...
	void SpawnManager::InitializeSprites()
	{
		for (std::uint32_t index = 0; index < mMaxSpawns; ++index)
		{
			XMFLOAT2 position(mRandomRealDistribution(mRandomGenerator), mRandomRealDistribution(mRandomGenerator));
			Transform2D transform(position, 0.0f, SpriteScale);
			auto sprite = make_shared<Sprite>(XMINT2(mRandomIntDistribution(mRandomGenerator), 0), transform);
